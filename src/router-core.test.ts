...
describe("canonical model routing", () => {
  ...
  it("draws capability numbers from the active source, never merged", () => {
    const ramp = resolveCanonicalModel("magi/kimi-k2.7-code", "ramp");
    ...
    const aa = resolveCanonicalModel("magi/kimi-k2.7-code", "aa");
    ...
    expect(resolveCanonicalModel("magi/gemini-3.5-flash", "ramp").supported).toBe(false);
    expect(resolveCanonicalModel("magi/gemini-3.5-flash", "aa").supported).toBe(true);
  });
  ...
  it("filters models by provider/id/name/canonical substring", () => {
    const pool = buildAutoPool(
      ...
      { ...AA, modelFilter: { include: ["magi"], exclude: [] } },
    );
  ...
  it("drops models the active source has no data for from the auto-pool", () => {
    ...
    const noRamp = [model("magi", "gemini-3.5-flash")];
  ...
  it("climbs the Ramp frontier by hardness on real resolve-rate (default source)", () => {
    const pool = buildAutoPool([
      model("magi", "gpt-5.4-nano"),
      model("magi", "qwen3.7-plus"),
      model("magi", "qwen3.6-plus"),
      model("magi-codex", "gpt-5.4"),
      model("magi", "kimi-k2.7-code"),
      model("magi-codex", "gpt-5.5"),
  ...
  it("keeps one routing key for tool continuations within the same user turn", () => {
    ...
    const nextUser = {
      messages: [
        ...
        { role: "assistant", content: [{ type: "text", text: "done" }], api: "openai-completions", provider: "magi", model: "deepseek-v4-flash", usage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, totalTokens: 0, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 } }, stopReason: "stop", timestamp: 4 },
